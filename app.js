...
function renderImage() {
    ...
    // Set canvas size
    elements.dicomCanvas.width = cols;
    elements.dicomCanvas.height = rows;
    ...
    const imageData = elements.ctx.createImageData(cols, rows);
    ...
    if (state.roiCenter) {
        ...
        elements.ctx.arc(state.roiCenter.x, state.roiCenter.y, state.roiRadius, 0, 2 * Math.PI);
        elements.ctx.stroke();

        // Draw crosshair
        elements.ctx.beginPath();
        elements.ctx.moveTo(state.roiCenter.x - 10, state.roiCenter.y);
        elements.ctx.lineTo(state.roiCenter.x + 10, state.roiCenter.y);
        elements.ctx.moveTo(state.roiCenter.x, state.roiCenter.y - 10);
        elements.ctx.lineTo(state.roiCenter.x, state.roiCenter.y + 10);
        elements.ctx.stroke();
    }
...
function getCanvasCoordinates(e) {
    ...
    const scaleX = elements.dicomCanvas.width / rect.width;
    const scaleY = elements.dicomCanvas.height / rect.height;
